...
폴더를 훑을 때 어떤 확장자를 검사 대상으로 수집할지는 설정 파일의 `input.video_extensions`에서 지정합니다. 사업 납품 포맷이 확정되면 이 목록만 조정하면 되고, 대소문자나 앞의 점(`.`) 유무는 가리지 않습니다. 폴더 대신 파일 하나를 직접 지정하면 확장자와 상관없이 그 파일을 검사합니다.

```yaml
input:
  # 이 확장자만 검사 대상으로 수집(기본값)
  video_extensions: [mxf, mov, mkv, avi, mp4, m4v, mpg, mpeg, ts, m2ts, wmv, vob, dv]
```

예를 들어 보존마스터(MXF)만 검사하려면 `video_extensions: [mxf]`로 두고 `--spec`으로 그 설정 파일을 지정합니다. 컨테이너·코덱·해상도 같은 납품 규격, 화질·음질 임계값, 파일명 규칙, 납품목록·메타데이터 대조 컬럼도 모두 같은 설정 파일에서 관리하므로, 감리 기준을 코드 수정 없이 사업 규격에 맞춰 바꿀 수 있습니다.
